...
$packages = "sounddevice", "numpy", "requests"
foreach ($package in $packages) {
    python -m pip install $package --quiet
    if ($?) {
        Write-Host "$package was installed successfully!"
    } else {
        Write-Host "Failed to install $package."
        exit 1
    }
}
Write-Host "All required Python packages have been installed successfully."
